  ...
  <div class="flex flex-col p-4 gap-4 items-center">
    <div id="mobile-menu__items">
      <span
        ...
        @click="navigate('/#block-pellicle-pools')"
      >
      ...
      >
        ...
        <fa-icon
          :class="{ 'rotate-180': expanded === 'equipment' }"
          icon="fa-solid fa-angle-up"
        />
      </span>
      <div v-if="expanded == 'equipment'" class="flex flex-col gap-4 p-2">
        <span @click="navigate('/catalog/plenka')"> Пленка для бассейна </span>
        <span @click="navigate('/catalog/nasosy')"> Насосы </span>
        <span @click="navigate('/catalog/filters')"> Фильтры </span>
        <span @click="navigate('/catalog/zakladnoe_oborudovanie')">
          Закладное оборудование
        </span>
        <span @click="navigate('/catalog/podogrev_vody')"> Подогрев воды </span>
        ...
        <span @click="navigate('/catalog/osveshenie')"> Освещение </span>
        <span @click="navigate('/catalog/lestnicy')"> Лестницы </span>
        <span @click="navigate('/catalog/truby')"> Трубы и фитинги </span>
      ...
      <span
        class="flex justify-center items-center"
        @click="
          () => (expanded == 'spa' ? (expanded = null) : (expanded = 'spa'))
        "
      >
        SPA и сауны
        <fa-icon
          :class="{ 'rotate-180': expanded === 'spa' }"
          icon="fa-solid fa-angle-up"
        />
      </span>
      <div v-if="expanded == 'spa'" class="flex flex-col gap-4 p-2">
        <span @click="navigate('/catalog/spa')"> SPA </span>
        <span @click="navigate('/catalog/sauny')"> Сауны </span>
      </div>
      <hr />
      <span>
        <a class="text-black" href="http://dpksklad.kz/"> Все для террас </a>
      </span>
      <hr />
      <span @click="navigate('/page/portfolio')"> Портфолио </span>
      <hr />
    ...
    <div class="flex gap-4">
      ...
      <a
        class="hover:opacity-75"
        href="https://www.instagram.com/abbex.kz/"
        target="_blank"
        rel="noreferrer"
      >
