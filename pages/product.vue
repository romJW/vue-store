      ...
      <div class="container lg:px-0 mx-auto">
        <div class="mx-auto flex flex-wrap">
          ...
          <div class="lg:w-1/2 w-full lg:pl-10 mt-6 lg:mt-0">
            ...
            <p v-if="data.deal_info" v-html="data.deal_info" class="prose max-w-none">
            </p>
            <hr class="my-4"/>
            <!--
...
export default {
  ...
  computed: {
     breadcrumbs () {
      if (this.product.value) {
        ...
        path.pop()
        return [
          { label: this.product.value.category.label, path: path.join('/') },
          { label: this.product.value.name, path: this.$route.path }
        ]
      }
    }
  },
  methods: {
    addToCart() {
      this.$store.commit('cart/add', product.value)
    },
  }
}
</script>
